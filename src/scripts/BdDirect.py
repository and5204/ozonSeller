...
class BdDirect:
    def __init__(self, db_path="requestsDirect.db"):
        ...
        self.cursor.execute("""
        CREATE TABLE IF NOT EXISTS requestsDirect (
    ...
    def _make_space(self):
        ...
        self.cursor.execute("""
        SELECT id, status FROM requestsDirect
        ORDER BY id ASC
        """)
        ...
        for row in rows:
            ...
            if status != "in_progress":
                # Удаляем эту запись
                self.cursor.execute("DELETE FROM requestsDirect WHERE id = ?", (req_id,))
                self.conn.commit()
                return True
    ...
    def get(self, request_id: int):
        self.cursor.execute("""
        SELECT * FROM requestsDirect WHERE id = ?
        """, (request_id,))
